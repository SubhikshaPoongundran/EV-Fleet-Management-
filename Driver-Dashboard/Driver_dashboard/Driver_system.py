...
BUZZER_PIN = 23 # Connect Positive pin to GPIO 23, Negative to GND
...
def sound_buzzer(alert_type):
    ...
    """ Runs on a separate thread to buzz without pausing the camera """
    ...
    try:
        ...
        if alert_type == "distraction":
            ...
            # 1 Quick Chirp for Auth Success
...
def on_connect(client, userdata, flags, reason_code, properties=None):
    ...
    if reason_code == 0:
        ...
        print(f"âœ… Connected to Server at {MQTT_BROKER}")

    else:

        print(f"âŒ MQTT Connection Failed: {reason_code}")
...
while True:
    ...
    try:
        ...
        print("â³ Connection timed out. Retrying in 5s... (Is Tailscale active?)")

        time.sleep(5)

    except Exception as e:

        print(f"âŒ Connection error: {e}. Retrying in 5 seconds...")
...
YOLO_ALERT_COOLDOWNS = {} # Tracks YOLO cooldowns



# --- ARC FACE & DB SETUP ---

print("ðŸ”„ Step 1: Loading ArcFace...")
...
def load_known_faces():
    ...
    if not os.path.exists(known_faces_path):
        ...
        print(f"âš ï¸ Created '{known_faces_path}' folder. Please place driver images inside.")
    ...
    print(f"ðŸ“ Database: {len(FACE_DB)} driver(s) loaded.")
...
print("ðŸ”„ Step 2: Loading Dlib Landmarks...")

if not os.path.exists("68 face landmarks.dat"):

    print("âŒ Error: Missing '68 face landmarks.dat'. Please download it.")
...
print("ðŸ”„ Step 3: Loading YOLOv8 Model...")
...
if os.path.exists("best.onnx"):
    ...
    print("âš ï¸ Warning: 'best.onnx' not found.")

    yolo_model = None



print("ðŸ”„ Step 4: Initializing Webcam...")
...
while True:
        ...
        for face in faces_fa:
            ...
            for db_name, db_emb in FACE_DB.items():
                ...
                if sim > AUTH_THRESHOLD:
                    ...
                    print(f"âœ… Welcome {db_name}! Unlocking vehicle...")
    ...
    if yolo_model is not None:
        ...
        yolo_results = yolo_model.predict(source=frame_display, conf=0.50, verbose=False)

        frame_display = yolo_results[0].plot() # Draw YOLO boxes

        

        detected_class_indices = [int(cls) for cls in yolo_results[0].boxes.cls]
        ...
        for obj in detected_class_names:
            ...
            # EMERGENCY BEEP: No Seatbelt

            if obj.lower() in ["no_seatbelt", "no seatbelt"]:

                if current_time - YOLO_ALERT_COOLDOWNS.get("seatbelt", 0) > 5.0:
            ...
            # EMERGENCY BEEP: Phone Usage

            if obj.lower() in ["phone", "using_phone", "cellphone"]:

                if current_time - YOLO_ALERT_COOLDOWNS.get("phone", 0) > 5.0:
            ...
            # DISTRACTION BEEP: Looking away

            if obj.lower() in ["distracted"]:

                if current_time - YOLO_ALERT_COOLDOWNS.get("distracted", 0) > 5.0:
...
# Moved to bottom to prevent blocking initialization
...
def gps_telemetry_loop():
    ...
    print(f"ðŸš€ Starting Direct Hardware GPS Access on {GPS_PORT}...")
