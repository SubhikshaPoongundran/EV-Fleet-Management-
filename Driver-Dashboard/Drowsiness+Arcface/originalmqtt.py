...
import os
import json
import paho.mqtt.client as mqtt # Added MQTT
from insightface.app import FaceAnalysis
from sklearn.metrics.pairwise import cosine_similarity

# --- MQTT CONFIGURATION ---
MQTT_BROKER = "broker.emqx.io"
MQTT_PORT = 1883 # Port 1883 for Python (TCP)
TOPIC_ALERTS = "taxi/app/driver/requests" # Topic your HTML is already listening to

mqtt_client = mqtt.Client()

def on_connect(client, userdata, flags, rc):
    if rc == 0:
        print("📡 Connected to MQTT Broker for Dashboard Alerts")
    else:
...
def send_to_dashboard(alert_type, message, status="danger"):
    """
    Sends a formatted JSON payload to the driver.html file.
    """
    payload = {
        "clientName": alert_type, # Using clientName field for Alert Title
        "fare": "SYSTEM ALERT",
